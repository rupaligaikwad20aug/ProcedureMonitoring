...
private:
	DWORD PID;
	HANDLE hThreadSnap;						// HANDLE  unsigned int
	THREADENTRY32 te32;						// inbuilt structure   
...
{
	if (!Thread32First(hThreadSnap, &te32))				// if true the returns 0   goes inside if    if it doesnt get first thread n print error.
	{
...
BOOL DLLInfo::DependentDLLDisplay()
{
	...
	{
		cout << "Failed to get DLLInformation" << endl;
...
BOOL ProcessInfo::ProcessLog()
{
	...
	else
	{
		...
		cout << "Time of logfile creation is->" << lt.wHour << ":" << lt.wMinute << ":" << lt.wDay << "th" << month[lt.wMonth - 1] << endl;
	}

	if (!Process32First(hProcessSnap, &pe32))				// if true the returns 0   goes inside if    if it doesnt get first thread n print error.
	{
...
{
	char *montharr[] = { "JAN", "FEB", "MAR", "APR", "MAY", "JUN", "JULY", "AUG", "SEP", "OCT", "NOV", "DEC" };
...
//	sprintf_s(FileName, "C://MarvellousLog %02d_%02d_%02d %s.txt", lt.wHour, lt.wMinute, lt.wDay, month[lt.wMonth - 1]);
	...
	if (fp == NULL)
	{
		cout << "Unable to create logfile named as :" << FileName<<endl;
		return FALSE;
	}
...
BOOL ProcessInfo::KillProcess(char *name)
{
	...
	if (hprocess == NULL)
	{
		cout << "There is no access to terminate" << endl;
		return FALSE;
	}
...
BOOL HardwareInfo()
{
	...
	cout << "Active processor mark:" << siSysInfo.dwActiveProcessorMask << endl;
	return TRUE;
}
...
void DisplayHelp()
{
	...
	cout << "clear : Clear the contents on console" << endl;
	cout << "log : Creates log of current running process on c drive"<< endl;
	cout << "readlog : Display information from specified log file"<< endl;
	cout << "sysinfo : Display the current hardware configuration" << endl;
	cout << "search : Serach and display information of specific running process" << endl;
